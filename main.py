...
def has_sufficient_data(freq):
    ...
    return freq in dataRTA and len(dataRTA[freq]) >= 10

def find_closest_frequency_with_data(target_frequency, band):
    """Finds the closest frequency with sufficient data in the specified band."""
    valid_frequencies = [(osc, freq) for osc, freq in band_ranges[band] if has_sufficient_data(freq)]
    if not valid_frequencies:
        return None, None  # If no valid frequency exists
    # Find the frequency closest to the target
    closest_osc, closest_freq = min(valid_frequencies, key=lambda x: abs(x[1] - target_frequency))
    return closest_osc, closest_freq

# Dictionary comprehension to initialize closest_frequencies
closest_frequencies = {
    band: [(osc, freq, find_closest_frequency_with_data(freq, band)) for osc, freq in freq_list]
    for band, freq_list in band_ranges.items()
}

# Adjusting to use the returned values correctly
for band, freq_list in closest_frequencies.items():
    for osc, original_freq, (closest_osc, closest_freq) in freq_list:
        if closest_freq is not None:
            print(f"Band: {band}, Original Freq: {original_freq}, Closest Freq ID: {closest_osc}, Frequency: {closest_freq}")
        else:
            print(f"Band: {band}, Original Freq: {original_freq}, No close frequency with sufficient data")
...
def calculate_gain(db_value, band, vocal_type):
    ...
    return round(gain, 2) # Round to two decimal places

def calculate_q_value(band, freq):
    """Calculate the Q value based on adjacent frequencies, only if the frequency has sufficient data."""
    ...
    adjacent_values_count = sum(1 for db in freq_db_values[max(0, freq_index - 1): freq_index + 2]
